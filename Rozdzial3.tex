...
Przekształcenie Fouriera dla sygnałów ciągłych służy do wyznaczania widma częstotliwościowego sygnału czasu ciągłego $x(t)$ $X(j\omega)$ \cite{ZielinskiDSP}. Wzory przekształcenia prostego i odwrotnego definiuje się następująco \cite{ZielinskiDSP}:
...
Aby całka określająca $X(j\omega)$ była zbieżna, sygnał $x(t)$ musi spełniać tzw. warunki Dirichleta: być bezwzględnie całkowalny, mieć skończoną liczbę ekstremów oraz skończoną liczbę punktów nieciągłości w dowolnym skończonym przedziale \cite{ZielinskiDSP}.
...
W systemach cyfrowych przetwarzany sygnał dostępny jest w postaci skończonego ciągu próbek $x(n)$, $n = 0, 1, \dots, N-1$, pobranych z częstotliwością próbkowania $f_{pr}$. Powyższy wzór na ciągłą transformatę Fouriera nie da się więc bezpośrednio zastosować. Zamiast niego widmo sygnału wyznacza się za pomocą dyskretnej transformacji Fouriera (DFT). Matematyczne przekształcenia dla prostej i odwrotnej DFT definiuje się następująco \cite{ZielinskiDSP}:
...
Wygodnym uproszczeniem powyższych zapisów jest wprowadzenie tzw. czynnika obrotu (ang. \emph{twiddle factor})\cite{CormenIntroductionAlgorithms}:
...
W inżynierii oprogramowania do porównywania wydajności algorytmów powszechnie wykorzystuje się notację dużego O, opisującą tempo wzrostu liczby wykonywanych operacji wraz ze długości wektora przetwarzanych danych wejściowych $N$. Zapis $O(g(N))$ oznacza, że liczba wykonywanych operacji rośnie nie szybciej niż funkcja $g(N)$ pomnożona przez pewną stałą, dla odpowiednio dużych $N$ \cite{CormenIntroductionAlgorithms}.
...
W pracy Cooleya-Tukeya \cite{CooleyTurkeyFFT} zaproponowano nowe podejście do obliczania dyskretnej transformaty Fouriera, polegające na rozłożeniu długiego sygnału, o $N$ próbkach, no kilka mniejszych grupy. Następnie policzono transformatę osobno dla każdej z nich, by w ostatnim kroku połączyć wyniki w widmo całego sygnału.

Dzięki temu jedno duże, kosztowne obliczenie DFT dla całego sygnału można zastąpić serią mniejszych obliczeń, wykonywanych etapami. Zamiast $N^2$ operacji, potrzebnych przy bezpośrednim liczeniu DFT, liczba operacji rośnie wtedy znacznie wolniej — w najlepszym przypadku mamy doczynienia ze złożonościa obliczeniową O($N\log N$). 

Autorzy zauważyli też, że wybór $N$ będącego potęgą liczby 2 lub 4 jest szczególnie wygodny dla komputerów działających na liczbach binarnych. Co więcej, cały algorytm można było wykonać na tej samej tablicy danych, bez potrzeby rezerwowania dodatkowej pamięci na wyniki pośrednie. Metoda ta stała się inspiracją dla tworzenia kolejnych wariantów tak zwanej szybkiej transformaty Fouriera. 

Do dalszej analizy teoretycznej posłuży popularny wariant FFT oparty na podstawie 2, czyli podziale na dwa podzbiory, jako najprostszy do intuicyjnego wyjaśnienia.

Podział wektora wejściowego może być przeprowadzony na dwa podstawowe sposoby. W wariancie z decymacją w czasie (DIT, ang. \emph{Decimation in Time}) dzieli się próbki sygnału wejściowego na te o indeksach parzystych i nieparzystych, wykonuje DFT osobno na każdym zbiorze, a następnie odtwarza widmo całego sygnału z widm cząstkowych \cite{ZielinskiDSP}. W wariancie z decymacją w częstotliwości (DIF, ang. \emph{Decimation in Frequency}) postępuje się odwrotnie — dzieli się nie próbki wejściowe, lecz prążki widma wyjściowego \cite{ZielinskiDSP}.

Konsekwencją podziału jest to, że dane trafiają do obliczeń w innej kolejności niż ta pierwotna w sygnale wejsciowym, więc na którymś etapie algorytmu należy je uporządkować. W algorytmach DIT robi się to na wejściu, przed właściwymi obliczeniami, i wynik otrzymuje się w kolejności naturalnej \cite{NumericalRecipesC}. W DIF jest odwrotnie — poprzestawiany jest dopiero wynik, na co zwrócono uwagę już w oryginalnej pracy \cite{CooleyTurkeyFFT}.

Sam algorytm można zapisać zarówno rekurencyjnie, jak i iteracyjnie — w praktyce spotyka się obie wersje \cite{NumericalRecipesC}.Rekurencyja dobrze oddaje ideę podziału na coraz mniejsze podproblemy, natomiast w implementacjach nastawionych na powtarzalność (oraz w tych wykonywanych w systemach, które mają ograniczoną pamięć) częściej stosuje się wariant iteracyjny, w którym zamiast rekurencyjnych wywołań wykonuje się pętlę po kolejnych etapach, łączącą coraz większe widma cząstkowe. Dla sygnału o długości $N$ takich etapów jest $\log_2 N$ \cite{NumericalRecipesC}.

Zarys algorytmu szybkiej transformaty Fouriera, w wariancie , opisany w pozycji \cite{NumericalRecipesC} przedstawiono na  \cref{alg:fft-dit-iteracyjny}:

\begin{algorithm}[h]
\caption{Iteracyjny algorytm FFT radix-2 z decymacją w czasie (DIT)}
\label{alg:fft-dit-iteracyjny}
\begin{algorithmic}[1]
\Function{FFT}{$x$, $N$}
    \State $x \gets$ \Call{PrzestawBitowoOdwrocone}{$x$, $N$} \Comment{bit-reversal wejścia}
    \State $etap \gets 2$
    \While{$etap \leq N$}
        \State $polowa \gets etap / 2$
        \State $W_{etap} \gets e^{-j\frac{2\pi}{etap}}$ \Comment{czynnik obrotu dla danego etapu}
        \For{$i = 0$ \textbf{do} $N - 1$ \textbf{krok} $etap$}
            \For{$k = 0$ \textbf{do} $polowa - 1$}
                \State $t \gets W_{etap}^k \cdot x[i + k + polowa]$
                \State $x[i + k + polowa] \gets x[i + k] - t$
                \State $x[i + k] \gets x[i + k] + t$
            \EndFor
        \EndFor
        \State $etap \gets etap \times 2$
    \EndWhile
    \State \Return $x$
\EndFunction
\end{algorithmic}
