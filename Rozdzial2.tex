...
>>>>>>> 150208d (Praca nad rozdzialem 2, cd)
...
Choć takie podejście zapewnia pełną kontrolę nad sprzętem i minimalne opóźnienia, znacznie ogranicza skalowalność systemu. Każda zmiana komponentu lub rozbudowa funkcjonalności może wymagać istotnych modyfikacji w strukturze kodu.
...
\textbf{TO JEST DO WYJEBANIA KONIEC}
...
W inżynierii oprogramowania istotną rolę odgrywa koncepcja \textbf{abstrakcji}, polegająca na  celowym ukryciu szczegółów implementacji danej funkcjonalności oprogramowania, przed jej potencjalnym użytkownikiem ~\cite{Tanenbaum}.
Podejście to opiera się na założeniu, że należy dostarczyć gotową funkcjonalność  wraz z odpowiednią dokumentacją opisującą sposób jej wykorzystania, zamiast wymagać od użytkownika zrozumienia wewnętrznych mechanizmów działania wytworzonego modułu.

Taka praktyka w sposób znaczący podnosi wygodę wytwarzania nowych elementów systemu, umożliwia zrównoleglenie wytwarzania oprogramowania oraz pozwala na większą specjalizację jej producentów. 
W celu realizacji komunikacji między kolejnymi modułami systemu stosuje się takie narzędzia jak \textbf{interfejsy programistyczne} (ang. \textit{Application Programming Interface}, skrót \textbf{API}), czyli zbiór ściśle określonych reguł i sposobów komunikacji, które umożliwają komunikację między różnymi warstwami oprogramowania.
Narzędzie to jest niezbędne, aby w sposób przewidywalny i bezpieczny przekazywać dane lub sterowanie, między różnymi elementami systemu.
...
Szczegółowe omówienie przytoczonych trybów oraz mechanizmów ich przełączania zostanie przedstawione w dalszych częściach tego rozdziału.

Warto przy tym podkreślić, że w przytoczonej pozycji literatury ~\cite{Abraham_Silberschatz} jako integralny komponent struktury systemu komputerowego uwzględnia się jego użytkownika. 
...
\item \textbf{Konieczność zapewnienia bezpieczeństwa } - Współczesne systemy operacyjne charakteryzują się ogromną złożonością, a ich kod źródłowy sięga kilkudziesięciu milionów linii ~\cite{Tanenbaum}.
 Izolacja poszczególnych poziomów wprowadza wyraźną hierarchię uprawnień. Dzięki temu błąd powstały w kodzie aplikacyjnym pozostaje odizolowany i nie wpływa bezpośrednio na stabilność pozostałych funkcjonalności systemowych ani na pracę samego sprzętu \cite{Abraham_Silberschatz}.
...
Zrozumienie tych bazowych koncepcji jest kluczowe dla analizy wydajności, ponieważ to one determinują sposób, w jaki program aplikacji, stworznej przez inżyniera oprogramowania, jest uruchamiany i wykonywany na fizycznym sprzęcie.
...
Ze względu na to, że pojedynczy rdzeń procesora może w danej chwili realizować tylko jedno zadanie, system operacyjny stosuje przełączanie kontekstu, cyklicznie przydzielając czas procesora poszczególnym procesom \cite{Tanenbaum}.
...
\textbf{Szeregowanie zadań} odpowiada za zarządzanie przydziałem czasu procesora. Pojedynczy rdzeń CPU nie jest w stanie wykonywać wielu zadań jednocześnie, dlatego \textbf{planista} (ang. scheduler) ustala kolejność oraz czas uruchamiania wątków i procesów na podstawie określonych algorytmów~\cite{Abraham_Silberschatz}.
...
\textbf{Pamięć wirtualna} sprawia, że procesy nie operują bezpośrednio na fizycznej pamięci operacyjnej, lecz na wirtualnej przestrzeni adresowej przydzielanej i mapowanej na sprzęt przez \textbf{jednostkę zarządzania pamięcią (MMU, ang. \textit{Memory Management Unit})}~\cite{Tanenbaum}. Należy w tym miejscu zaznaczyć, że jednym z warunków korzystania z systemów operacyjnych
jest obecność takiej jednostki w sprzęcie, na którym chcemy ten system uruchomić.
...
Mechanizm zarządzania pamięcią wirtualną został opisany, na przykładzie systemu Linux w sekcji \ref{Linux_wykorzystaniePamieci}
...
na których użytkownik może uruchamiać dowolne aplikacje, system wbudowany ma z góry zdefiniowaną funkcję i nie jest przeznaczony do późniejszych, swobodnych modyfikacji przez użytkownika końcowego~\cite{Abraham_Silberschatz}.

<<<<<<< HEAD
Szczególną grupą w obrębie systemów wbudowanych są \textbf{systemy wbudowane czasu rzeczywistego}. Są to takie systemy, których poprawność wyniku przetwarzania nie zależy tylko od poprawności logicznej odpowiedzi, ale też od czasu przetwarzania, poprzedzającego odpowiedź systemu~\cite{WangK}.
...
\end{itemize}
W celu lepszego zobrazowania podziału systemów czasu rzeczywistego, na rysunku \cref{fig:wykresy RTOS} przedstawiono wykresy funkcji przydatności odpowiedzi $y(t)$ od czasu jej wypracowania przez system ~\cite{Tanenbaum}. 
\begin{figure}[htbp]
\centering
...
Najbardziej restrykcyjne kryteria czasowe są charakterystyczne dla twardych systemów czasu rzeczywistego \cite{Tanenbaum}, co zobrazowano na rysunku \ref{fig:Hard RTOS wykres}.
...
W przypadku rozwiązań spóźniona odpowiedź jest nie tylko całkowicie bezużyteczna, ale jej pojawienie się po wyznaczonym terminie jest najczęściej równoznaczne z wystąpieniem krytycznego błędu, który może doprowadzić do poważnej awarii lub fizycznego uszkodzenia całego systemu~\cite{Abraham_Silberschatz}.
...
jak to możliwe, a ewentualne opóźnienia nie niosą za sobą skutków katastrofalnych \cite{Tanenbaum}. Jeśli odpowiedź zostanie dostarczona po czasie $t_1$, ale przed ostatecznym limitem $t_T$, jej przydatność maleje, powodując jedynie spadek jej jakości.
Przy przetwarzaniu przekraczającym czas $t_T$ przydatność odpowiedzi wynosi zero. 

Solidne systemy czasu rzeczywistego stanowią pewnego rodzaju kompromis pomiędzy twardymi a miękkimi systemami czasu rzeczywistego. Zachowanie funkcji przydatności systemów solidnych przedstawiono na rysunku \ref{fig:firm RTOS wykres}. 
...
Zdefiniowane powyżej rygory czasowe określają teoretyczne oczekiwania wobec systemu czasu rzeczywistego. W praktyce realizacja tych założeń sprowadza się do oceny zasadności stosowania \textbf{systemu operacyjnego czasu rzeczywistego} 
...
Główną wadą systemów monolitycznych pozostaje jednak słaba odporność na awarie samego jądra; jeśli błąd pojawi się w jego obrębie, na przykład w jednym ze sterowników, może dojść do awarii całego systemu.
...
Z kolei fundamentalną różnicą w architekturze systemu Linux, w porównaniu do klasycznych systemów UNIX, jest obsługa wątków. Wprowadzono w nim wywołanie systemowe \texttt{clone}, które zaciera tradycyjną różnicę między procesami a wątkami ~\cite{Tanenbaum}, umożliwiając tworzenie nowego wątku z bardzo podobnymi cechami do klasycznego procesu. 
...
W systemie Linux pamięć jest stronnicowana, w związku z czym podstawową jednostką zarządzania pamięcią fizyczną jest struktura o nazwie \textit{page} ~\cite{LinuxKernelDevelopment}. Z uwagi na ograniczenia sprzętowe niektórych architektur, pamięć fizyczna została logicznie podzielona na strefy (ang. \textit{zones}). Należą do nich między innymi \texttt{ZONE\_DMA}, przeznaczona na operacje bezpośredniego dostępu do pamięci, \texttt{ZONE\_NORMAL}, zawierająca standardowe strony, oraz \texttt{ZONE\_HIGHMEM} ~\cite{Tanenbaum}. 
...
Graficzne przedstawienie tej struktury oraz wzajemnych zależności pomiędzy poszczególnymi poziomami architektonicznymi zostało zaprezentowane na rysunku~\ref{fig:Model_Warstwowy}. 

Warstwa sprzętowa obejmuje samo fizyczne urządzenie oraz niekiedy zintegrowane z nim, najbardziej podstawowe mechanizmy i niskopoziomowe oprogramowanie 
układowe umożliwiające podstawowy dostęp do rejestrów i kontrolerów. Bezpośrednia interakcja z tak surowym środowiskiem sprzętowym jest jednak niezwykle złożona 
i uciążliwa z punktu widzenia programisty aplikacji. Aby uprościć sterowanie fizycznymi komponentami, stosuje się system operacyjny.
W architekturze systemów opartych na jądrze Linux fundamentalne znaczenie ma również podział na dwa odizolowane tryby pracy procesora: tryb użytkownika oraz tryb jądra. 
Zagadnienia te, wraz ze szczegółową charakterystyką przestrzeni użytkownika (ang. \textit{user space}) i przestrzeni jądra (ang. \textit{kernel space}), zostaną szczegółowo omówione
i przeanalizowane w późniejszych sekcjach tego rozdziału.

\begin{figure}[htbp]
    \centering % Środkuje grafikę na stronie
    % W opcjach [width=...] definiujesz szerokość, np. 0.8 szerokości tekstu
    \includegraphics[width=0.8\textwidth]{img/Rysunek1.drawio.pdf} 
    \caption{Model warstwowy systemów operacyjnych}
    \label{fig:Model_Warstwowy} % Etykieta do odwoływania się w tekście (np. \ref{fig:moje_zdjecie})
\end{figure}
...
% tutaj bedziemy pisac o ogólnych założeniach linuxa, jego wykorzystaniu, tego, że wyszedł z Unixa, ze jest openSource, etc
\subsection{Separacja przestrzni jądra systemu od przestrzeni użytkownika}
